...
smodel <- stan_model("consumption_probability_source.stan", verbose = F)
...
dbin$source_num <- as.numeric(dbin$source)
...
saveRDS(mp1, "exports/consumption_probability_model_source.rds")
mp1 <- readRDS("exports/consumption_probability_model_source.rds")

# perfect, 1 min.
pairs(mp1, pars = c("b", "a", "u"))
pairs(mp1, pars = c("sigma_plant", "sigma_bird", "sigma_source", "u"))

sm1 <- summary(mp1)[[1]]
min(sm1[, "n_eff"]) # 2882.79
max(sm1[, "Rhat"])  # 1.003681
...
sigma_source <- as.matrix(mp1, "sigma_source") %>% as.numeric
sigma_raneff <- rbind(
...
e_source_hat <- as.matrix(mp1, "e_source") %>% t
...
var_names <- c("a", "b", "u", "sigma_plant", "sigma_bird", "sigma_source")
...
X_source <- model.matrix(~ source - 1, dbin)
...
par(mfrow = c(2, 2))
plot(density(rowMeans(e_bird_hat), adjust = 1.5), main = "birds")
plot(density(rowMeans(e_plant_hat), adjust = 1.5), main = "plants")
plot(density(rowMeans(e_source_hat), adjust = 1.5), main = "source")
par(mfrow = c(1, 1))
...
mu_raw_logitnorm_pred <- logit_norm_mean(mu_raw_logit_pred,
                                         sigmas_mat_pred)
saveRDS(mu_raw_logitnorm_pred, "exports/consumption_probability_pred-mu-samples_source.rds")
mu_raw_logitnorm_pred <- readRDS("exports/consumption_probability_pred-mu-samples_source.rds")
...
ggsave("figures/consumption_probability_predictions_source.tiff",
       width = 1961, height = round(1961 * 0.55), units = "px",
       dpi = 300)
